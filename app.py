...
/* CUSTOM COLORED METRIC CLASS */
.metric-container {{ text-align: center; padding: 10px; }}
.metric-label {{ color: #94A3B8; font-size: 14px; margin-bottom: 4px; }}
...
# Helper function to keep tooltips AND colors
def custom_metric(label, value, color, help_text=""):
    st.markdown(f"""
        <div class="metric-container">
            <div class="metric-label">{label} <span title="{help_text}" style="cursor:help;">ⓘ</span></div>
            <div class="metric-value" style="color:{color};">{value}</div>
        </div>
    """, unsafe_allow_html=True)

# 2. DATA CONFIG & CRASH PROTECTION
...
# FIX: Reset ID if industry changes to prevent Crash
...
if not checked.empty:
    new_id = checked.iloc[-1]["Customer ID"]
    if new_id != st.session_state.selected_id:
        st.session_state.selected_id = new_id
        st.rerun()

# 4. SECTION 2: SIMULATION (WITH TOOLTIPS)
...
c1, c2 = st.columns(2)
with c1:
    tenure = st.number_input("Tenure (Months)", 1, 72, int(selected_row["tenure"]), help="Adjust loyalty duration impact.")
    contract = st.selectbox(cfg["label"], ["Standard", "Premium", "Enterprise"], help="Simulate tier changes.")
with c2:
    monthly = st.number_input("Monthly Value ($)", 1, 10000, int(selected_row["MonthlyCharges"]), help="Simulate revenue impact.")
...
# 5. RESULTS (COLORED + TOOLTIPS)
st.markdown("---")
m1, m2 = st.columns(2)
with m1: custom_metric("Simulated Risk", f"{sim_risk:.1f}%", NEON_BLUE, "Predicted churn probability.")
with m2: custom_metric("Revenue Safeguarded", f"+${savings:,.2f}", NEON_GREEN, "Total revenue protected.")

# 6. XAI & MACRO
st.markdown("---")
st.markdown('<p class="section-label">3. Explainable AI (XAI)</p>', unsafe_allow_html=True)
x1, x2 = st.columns(2)
with x1: custom_metric(f"{cfg['label']} Impact", "High" if contract=="Standard" else "Low", NEON_GREEN)
with x2: custom_metric("Support Impact", "High" if not has_support else "Low", NEON_GREEN)

st.markdown("---")
st.markdown('<p class="section-label">4. Macro Business Impact Projection</p>', unsafe_allow_html=True)
bi1, bi2, bi3 = st.columns(3)
with bi1: custom_metric("Annual Savings", f"+${(savings * 12 * (cfg['scale']/100)):,.0f}", NEON_GREEN)
with bi2: custom_metric("Efficiency", "91%", NEON_BLUE)
with bi3: custom_metric("Confidence", "94.2%", GOLD_COLOR)
