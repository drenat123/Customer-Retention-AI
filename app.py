...
# 2. THE STABLE UI ENGINE (ENHANCED CSS FOR TRANSPARENCY)
st.markdown("""
    <style>
    ...
    /* ULTIMATE FIX FOR BLACK BOXES */
    /* This targets the markdown containers that Streamlit uses for dynamic text */
    ...
    /* XAI DOTS */
    .dot-high { color: #FF4B4B; margin-right: 5px; }
    .dot-low { color: #00FFAB; margin-right: 5px; }
    </style>
...
# 🛠️ STRATEGY SANDBOX (TRANSPARENCY FIXED)
st.markdown("---")
...
# THE CLEAN BOX (REMOVED BACKGROUND INLINE)
st.markdown(f"""
...
# ---------------------------------------------------------
# NEW IMPLEMENTATION: FEATURE CONTRIBUTION (XAI)
...
with xai_c1:
    st.markdown(f"<p style='color: #94A3B8; font-size: 14px;'>{cfg['label']} Impact: <b style='color: white;'>{contract_impact}</b></p>", unsafe_allow_html=True)
with xai_c2:
    st.markdown(f"<p style='color: #94A3B8; font-size: 14px;'>Support Impact: <b style='color: white;'>{support_impact}</b></p>", unsafe_allow_html=True)
