...
# ==========================================
# 🎨 COLOR CONTROLS (Manual Override)
# ==========================================
NEON_BLUE = "#00F0FF"
NEON_GREEN = "#00FFAB"
GOLD_COLOR = "#FFD700"

# 2. THE "NO-STRIP" CSS ENGINE
st.markdown(f"""
    <style>
    ...
    /* TARGETING VALUES DIRECTLY TO PREVENT WHITE-OUT */
...
# Reset session state if switching industries to avoid IndexError
...
# 5. SECTION 2: SIMULATION LAB (RESTORED BUTTONS & TOOLTIPS)
...
c1, c2 = st.columns(2)
with c1:
    tenure = st.number_input("Tenure (Months)", 1, 72, value=int(selected_row['tenure']), help="Adjust the customer's loyalty duration to see exactly how time spent influences their likelihood to churn.")
...
with c2:
    monthly = st.number_input("Monthly Value ($)", 1, 10000, value=int(selected_row['MonthlyCharges']), help="The recurring monthly revenue. This allows you to simulate how price adjustments impact the total revenue safeguard.")
    has_support = st.checkbox("Simulate Priority Support?", value=True, help="Enable this to simulate the impact of adding a dedicated support agent to this specific account.")
...
sim_risk = max(5, base_risk - (sim_discount * 0.6))
savings = ((base_risk/100) * (monthly * 24)) - ((sim_risk/100) * ((monthly * (1 - sim_discount/100)) * 24))

st.markdown("---")
m1, m2 = st.columns(2)
with m1: st.metric("Simulated Risk", f"{sim_risk:.1f}%", help="The AI's predicted churn probability for this specific scenario.")
with m2: st.metric("Revenue Safeguarded", f"+${savings:,.2f}", help="Total dollar amount of revenue protected from loss based on the current strategy.")

# 6. SECTION 3: XAI
st.markdown("---")
st.markdown('<p class="section-label">3. Explainable AI (XAI)</p>', unsafe_allow_html=True)
x1, x2 = st.columns(2)
with x1: st.metric(f"{cfg['label']} Impact", "🔴 High" if contract == "Standard" else "🟢 Low", help="Shows how much the contract type pushes risk up or down.")
with x2: st.metric("Support Impact", "🔴 High" if not has_support else "🟢 Low", help="Indicates the correlation between priority support and customer retention.")
...
with bi1: st.metric("Annual Savings", f"+${(savings * 12 * (cfg['scale']/100)):,.0f}", help="Projected department-wide recovery.")
with bi2: st.metric("Efficiency", "91%", help="Historical accuracy of the AI model.")
with bi3: st.metric("Confidence", "94.2%", help="Statistical confidence level for the current prediction.")
