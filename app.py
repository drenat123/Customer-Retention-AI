...
# 2. THE "FORCE COLOR" ENGINE
# This uses 'data-testid' and direct attribute targeting to override the white color.
st.markdown("""
    <style>
    ...
    /* THE FIX: Forcing colors by targeting the specific metric value ID */
    ...
    div[data-testid="stMetric"]:has(label:contains("Impact")) [data-testid="stMetricValue"] {
...
# 6. SECTION 2: SIMULATION LAB (FULL TOOLTIPS)
...
m1, m2 = st.columns(2)
with m1:
    st.metric("Simulated Risk", f"{risk:.1f}%", help="The AI's predicted churn probability for this scenario. Lower is better.")
with m2:
...
x1, x2 = st.columns(2)
with x1:
    st.metric(f"{cfg['label']} Impact", "🔴 High" if contract == "Standard" else "🟢 Low", help="How much this factor is pushing the risk up or down.")
with x2:
    st.metric("Support Impact", "🔴 High" if not has_support else "🟢 Low", help="The correlation between priority support and retention.")
...
with bi2: 
    st.metric("Efficiency", "91%", help="Historical accuracy of the AI model.")
with bi3: 
    st.metric("Confidence", "94.2%", help="Statistical certainty of the current prediction.")
