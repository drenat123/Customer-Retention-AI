...
# 2. THE ULTIMATE COLOR ENGINE (TARGETING THEME CLASSES)
st.markdown("""
    <style>
    ...
    /* FORCING COLOR ON THE TEXT ITSELF BY TARGETING STREAMLIT'S INTERNAL DATA ATTRIBUTES */
    div[data-testid="stMetric"]:has(label:contains("Simulated Risk")) [data-testid="stMetricValue"],
    div[data-testid="stMetric"]:has(label:contains("Efficiency")) [data-testid="stMetricValue"] {
        color: #00F0FF !important;
        fill: #00F0FF !important;
    }
    
    div[data-testid="stMetric"]:has(label:contains("Revenue Safeguarded")) [data-testid="stMetricValue"],
    div[data-testid="stMetric"]:has(label:contains("Annual Savings")) [data-testid="stMetricValue"] {
        color: #00FFAB !important;
        fill: #00FFAB !important;
    }
...
# CRITICAL FIX: Ensure session_state ID exists in the NEW industry dataframe
...
# 6. SECTION 2: SIMULATION LAB (RESTORED TOOLTIPS)
# Safe row lookup
match = base_df[base_df['customerID'] == st.session_state.selected_id]
selected_row = match.iloc[0] if not match.empty else base_df.iloc[0]

st.markdown(f'<p class="section-label">2. Simulation Lab: {selected_row["customerID"]}</p>', unsafe_allow_html=True)
c1, c2 = st.columns(2)
with c1:
    tenure = st.number_input("Tenure (Months)", 1, 72, value=int(selected_row['tenure']), help="Adjust the customer's loyalty duration.")
    contract = st.selectbox(cfg['label'], ["Standard", "Premium", "Enterprise"], help=f"Simulate a change in the {cfg['label']}.")
with c2:
    monthly = st.number_input("Monthly Value ($)", 1, 10000, value=int(selected_row['MonthlyCharges']), help="The recurring monthly revenue.")
    has_support = st.checkbox("Simulate Priority Support?", value=True, help="Toggle the impact of dedicated support.")

# Retention Math
risk = 35 if contract == "Standard" else 10
if not has_support: risk += 15
risk = max(5, min(95, risk - (tenure * 0.3)))
savings = ((risk/100) * (monthly * 24))

st.markdown("---")
m1, m2 = st.columns(2)
with m1:
    st.metric("Simulated Risk", f"{risk:.1f}%", help="AI's predicted churn probability.")
with m2:
    st.metric("Revenue Safeguarded", f"+${savings:,.2f}", help="Revenue protected from loss.")
