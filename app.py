...
# 2. THE CSS ENGINE (LARGE UI & CENTERING)
st.markdown("""
    <style>
    ...
    /* FORCING LARGE CENTERED METRICS */
...
# 7. SECTION 2: SIMULATION LAB (TOOLTIPS ADDED HERE)
...
c1, c2 = st.columns(2)
with c1:
    tenure = st.number_input("Tenure (Months)", 1, 72, value=int(selected_row['tenure']), help="Adjust the customer's loyalty duration to see impact on risk.")
    contract = st.selectbox(cfg['label'], ["Standard", "Premium", "Enterprise"], help=f"Change the {cfg['label']} to simulate tier-based retention.")
with c2:
    monthly = st.number_input("Monthly Value ($)", 1, 10000, value=int(selected_row['MonthlyCharges']), help="The recurring revenue associated with this account.")
    has_support = st.checkbox("Simulate Priority Support?", value=(selected_row['OnlineSecurity'] == "Yes"), help="Toggle to see how dedicated support reduces churn probability.")
...
m1, m2 = st.columns(2)
with m1:
    st.metric("Simulated Risk", f"{sim_risk:.1f}%", help="Predicted churn probability after your simulation.")
with m2:
    st.metric("Revenue Safeguarded", f"+${savings:,.2f}", help="Revenue protected by your retention strategy.")
...
x1, x2 = st.columns(2)
with x1:
    st.metric(f"{cfg['label']} Impact", "🔴 High" if contract == "Standard" else "🟢 Low", help="Effect of the contract on the risk score.")
with x2:
    st.metric("Support Impact", "🔴 High" if not has_support else "🟢 Low", help="Correlation of support to retention.")
...
with bi1: 
    ...
    st.metric("Annual Savings", f"+${macro_val:,.0f}", help="Projected department-wide recovery.")
with bi2: 
    st.metric("Efficiency", "91%", help="AI Accuracy.")
with bi3: 
    st.metric("Confidence", "94.2%", help="Statistical confidence level.")
